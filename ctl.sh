...
_is_running()
{
    ...
    if [ -d /proc/$pid ] && [[ "$(cat /proc/$pid/cmdline | tr -d '\0')" == *"${cmd// /}"* ]]; then
        return 0
    else
        return 1
    fi
...
_reload()
{
    ...
    for ((i=${#SERVERS[@]}-1; i>=0; i--)); do
        ...
        pidfile=$svr_name.pid
        if [ -f $pidfile ]; then
            echo -n "$(printf '%-32s' "reloading $svr_name ...")"
            ...
            echo -e "\033[32m[RELOADED]\033[0m"
        fi
    done
}
...
case "$cmd" in
    ...
    *)
        echo "Usage: ctl.sh {start|stop|restart|status|reload} [server...]"
        echo
