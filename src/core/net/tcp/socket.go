...
type Socket struct {
	...
	rbuf_size int              // read buffer size
...
func (self *Socket) SetReadBufferSize(n int) {
	self.rbuf_size = n
}
...
func (self *Socket) SetSysReadBufferSize(n int) {
	if c, ok := self.c.(*net.TCPConn); ok {
		c.SetReadBuffer(n)
	}
}

func (self *Socket) SetSysWriteBufferSize(n int) {
	if c, ok := self.c.(*net.TCPConn); ok {
		c.SetWriteBuffer(n)
	}
}
